...
const keycloakPublicUrl = import.meta.env.VITE_KEYCLOAK_URL as string ?? keycloakUrl;
const clientId = process.env.KEYCLOAK_CLIENT_ID;
if (!keycloakUrl || !clientId) {
    throw new Error("Keycloak not configured");
}
const headers = {
    'Host': keycloakPublicUrl
};

export const keycloak : IdentityProvider = {
    buildAuthorizationUrl: (params) => {

        const url = new URL(keycloakPublicUrl + '/protocol/openid-connect/auth');
