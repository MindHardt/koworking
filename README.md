# АРХИВ
Этот репозиторий заархивирован, так как мы перешли на работу во внутреннем git-сервере.
Дальнейшая разработка будет вестись там, этот репозиторий остаётся для моего резюме.

# Koworking
Сайт для поиска постоянной или временной работы

# TODO:
- [x] utm-ссылки + clickhouse
- [ ] сокращённые ссылки на приложение
- [x] аутентификация
- [ ] создание вакансий
- [ ] отклик на вакансии
- [ ] чат в приложении

# Начало разработки
Чтобы поднять все нужные сервисы используйте
```shell
docker compose up
```

Если вы поднимаете сервисы впервые на этой 
машине то импортируйте реалм по 
[инструкции](./keycloak/README.md).

Далее остановите контейнер с которым будете 
работать и разверните его локальную копию. Шикарно!
